...
@app.post("/webhook")
async def hfish_webhook(request: Request):
    try:
        data = await request.json()
        logger.info(f"Webhook received: {data}")
        
        # HFish sends attack info in various formats depending on version/config
        # We try to find the IP in common fields
        ip = data.get("attack_ip") or data.get("src_ip") or data.get("ip")
        
        if not ip and "data" in data:
            if isinstance(data["data"], dict):
                ip = data["data"].get("attack_ip") or data["data"].get("src_ip") or data["data"].get("ip")
            elif isinstance(data["data"], list) and len(data["data"]) > 0:
                ip = data["data"][0].get("attack_ip") or data["data"][0].get("src_ip")
        
        if ip:
            # Store local data for 365 days
            REDIS_CLIENT.setex(f"{KEY_LOCAL}{ip}", timedelta(days=365), datetime.now().isoformat())
            logger.info(f"Successfully recorded local intelligence for IP: {ip}")
            return {"status": "ok", "recorded_ip": ip}
            
        return {"status": "ignored", "reason": "no ip found"}
    except Exception as e:
